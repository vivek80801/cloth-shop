...
const Black: React.FC = (): JSX.Element => {
  ...
  const blackProducts = [
    ...products.products.filter(
      (product) =>
        product.sub_category === "Black" && product.category === "woman"
    ),
  ];
  const pagenumbers = [];
  const indexOfLastProducts = currentPage * productPerPage;
  const indexOfFirstProducts = indexOfLastProducts - productPerPage;
  const currentProduct = blackProducts.slice(
    indexOfFirstProducts,
    indexOfLastProducts
  );

  for (let i = 1; i <= Math.ceil(blackProducts.length / productPerPage); i++) {
    pagenumbers.push(i);
  }
  return (
    <>
          ...
          <div className="span9">
                  ...
                  <li className="span3" key={newProduct.id}>
                    <div className="product-box">
                      ...
                      <Link to={`/details/${newProduct.id}`}>
                        <img alt={newProduct.title} src={`../${newProduct.img}`} />
                      </Link>
                      <br />
            ...
            <div className="pagination pagination-small pagination-centered">
              <ul>
                ...
                {currentPage <
                Math.ceil(blackProducts.length / productPerPage) ? (
                  <li>
                    <Link
                      onClick={() => setCurrentPage(currentPage + 1)}
                      to="#"
